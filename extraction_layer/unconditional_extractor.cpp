#include "unconditional_extractor.hpp"
...
ConditionAggregate ReadAggregateBinary(const std::string& path) {
    ...
    for (std::size_t i = 0; i < aggregate.accumulated_mass.size(); ++i) {
        input.read(reinterpret_cast<char*>(&aggregate.accumulated_mass[i]), sizeof(double));
    }
...
UnconditionalExtractor::UnconditionalExtractor(const std::string& artifact_root,
                                               bool emit_debug_bundle_counts)
    : artifact_root_(artifact_root),
      emit_debug_bundle_counts_(emit_debug_bundle_counts) {}

ConditionAggregate UnconditionalExtractor::run(const CorpusDataset& dataset,
    ...
    aggregate.conditional_mode = false;
    ...
    for (std::size_t i = 0; i < samples.size(); ++i) {
        if (progress_emitter && (i == 0 || i % std::max<std::size_t>(1, samples.size() / 10U) == 0U)) {
            progress_emitter->emit("Unconditional extraction processing sample " +
        ...
        update_aggregate(stats, &aggregate);
    ...
    if (progress_emitter) {
        progress_emitter->emit("Unconditional extraction complete for " + condition.condition_id, 100);
    }
    return aggregate;
}

void UnconditionalExtractor::update_aggregate(const SampleBundleStats& stats,
    ...
    for (std::size_t feature_id = 0; feature_id < stats.feature_counts.size(); ++feature_id) {
        ...
        aggregate->accumulated_mass[feature_id] += raw_count;
        if (raw_count > 0U) {
            ++aggregate->extracted_sample_counts[feature_id];
        }
        aggregate->extraction_telemetry.total_observed_mass += raw_count;
        aggregate->extraction_telemetry.total_passed_mass += raw_count;
    }
}

void UnconditionalExtractor::write_aggregate_artifact(const ExperimentCondition& condition,
...
ConditionAggregate UnconditionalExtractor::load_aggregate_artifact(const std::string& artifact_path) const {
