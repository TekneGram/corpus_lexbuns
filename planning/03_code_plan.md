...
The first implementation should support only the core experiment path:

- 4-gram bundles only
- composition modes `short`, `mid`, `long`
- conditional and unconditional modes
- document-dispersion thresholds only
- final robust set construction
- pairwise Jaccard comparison
- CLI and JSON entrypoints
- progress emission

It should also support these user-control features from the start:
...
- semantic binary sidecars
- exact position reconstruction during the main experiment loop
- parallel execution across threads

Those can be added later once the single-process experimental pipeline is stable.
